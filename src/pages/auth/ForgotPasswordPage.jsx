      ...
      <Card className="w-full max-w-sm p-8 border-0 shadow-lg">
        ...
        {sent ? (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">Un email de réinitialisation a été envoyé.</p>
            <Link to="/connexion"><Button variant="outline" className="gap-2"><ArrowLeft className="h-4 w-4" />{t('auth.back_login')}</Button></Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><Label>{t('auth.email')}</Label><Input type="email" value={email} onChange={e => setEmail(e.target.value)} className="mt-1" required /></div>
            <Button type="submit" className="w-full">{t('auth.send_reset')}</Button>
            <Link to="/connexion" className="flex items-center justify-center gap-1 text-xs text-primary hover:underline"><ArrowLeft className="h-3 w-3" />{t('auth.back_login')}</Link>
          </form>
        )}
      </Card>
    </div>
