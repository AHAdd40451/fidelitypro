...
export default function SignupPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate('/merchant/dashboard'); }, 1000);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-background to-accent/30">
      ...
      <Card className="w-full max-w-lg p-8 border-0 shadow-lg my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><Label>{t('auth.password')}</Label><Input type="password" value={form.password} onChange={e => update('password', e.target.value)} className="mt-1" required /></div>
            <div><Label>{t('auth.confirm_password')}</Label><Input type="password" value={form.confirmPassword} onChange={e => update('confirmPassword', e.target.value)} className="mt-1" required /></div>
          </div>
          <div>
          ...
          <div>
            ...
            <Input value={form.welcomeMsg} onChange={e => update('welcomeMsg', e.target.value)} className="mt-1" placeholder="Bienvenue chez..." />
          </div>
          {slug && (
          ...
          <Button type="submit" className="w-full" disabled={loading}>{loading ? t('common.loading') : t('auth.signup_btn')}</Button>
        </form>
        <p className="text-center text-xs text-muted-foreground mt-4">
          {t('auth.have_account')} <Link to="/connexion" className="text-primary hover:underline">{t('auth.login_link')}</Link>
        </p>
      </Card>
    </div>
  );
